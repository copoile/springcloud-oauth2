...
资源服务：拦截请求后会远程调用认证服务器的`http://127.0.0.1:8001/user或http://127.0.0.1:8001/oauth/check_token，至于调用哪个取决于配置文件，如配置如下配置将远程调用http://127.0.0.1:8001/user（资源服务端我们也一般这么配置即可)`
```yml
##安全配置##
security:
  oauth2:
